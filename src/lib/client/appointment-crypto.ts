...
export class UnifiedAppointmentCrypto {
  ...
  async encryptTunnelKeyForStaff(
    ...
    if (!this.tunnelKey && !externalTunnelKey) throw new Error("No tunnel key available");

    // Export tunnel key as raw bytes
    const tunnelKeyBytes = await crypto.subtle.exportKey(
      "raw",
      externalTunnelKey ?? this.tunnelKey!, // TODO
    );
